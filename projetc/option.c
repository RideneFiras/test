    			...
    			switch(event.key.keysym.sym)
    			{
					...
					case SDLK_RETURN:
						...
						{SDL_BlitSurface(sure[2],NULL,ecran,&pos);
    						SDL_Flip(ecran);
    				SDL_Delay (50);								ecran=SDL_SetVideoMode(1000,600,32,SDL_HWSURFACE |SDL_DOUBLEBUF );
    				return 0;
						}
						else if (n==1)//no
...
SDL_Surface *background,*boutton_sfx[6],*boutton_music[6],*full_screen[3],*keys[3],*music[2],*sfx[2],*back[2];
	int m=1,s=0,ks=0,fs=0,t,n=1,q=1,p_sfx,volume_sfx,p_music,volume_music,mp=0,bk=0;
	...
	pos.x=0;
	pos.y=0;
	back[0]=IMG_Load("graphic/option/back1.png");
	back[1]=IMG_Load("graphic/option/back2.png");
...
Mix_PlayMusic(music_sound,-1);
	...
	SDL_BlitSurface(back[bk],NULL,screen,&pos);
        SDL_Flip(screen);
